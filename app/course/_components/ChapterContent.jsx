...
const enrollCourse = courseInfo?.enrollCourse;
  ...
  return (
    <div
      className={`${isCollapsed ? "ml-17" : "ml-17 lg:ml-80"} transition-all duration-300`}
    >
      {/* Main content container with responsive width */}
      <div
        className={`${isCollapsed ? "max-w-7xl mx-auto px-4 md:px-8" : ""} p-4 md:p-8 lg:p-10 xl:p-12 space-y-6 md:space-y-8 w-full`}
      >
        {/* Chapter Title Section */}
        <div
          className={`${isCollapsed ? "max-w-6xl mx-auto" : ""} flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4`}
        >
        ...
        <div
          className={`${isCollapsed ? "max-w-6xl mx-auto" : ""} h-px bg-border w-full`}
        />

        {/* Related Videos Section - Responsive columns */}
        {includeVideo && videoData?.length > 0 && (
          <div
            className={`${isCollapsed ? "max-w-6xl mx-auto" : ""} space-y-4`}
          >
            ...
            <div
              className={`grid ${isCollapsed ? "grid-cols-1 lg:grid-cols-3 gap-6" : "grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6"}`}
            >
        ...
        {/* Topics Section */}
        <div
          className={`${isCollapsed ? "max-w-6xl mx-auto" : ""} mt-8 md:mt-10 lg:mt-12 space-y-6 md:space-y-8 lg:space-y-10`}
        >
          {topics?.map((topic, index) => (
            <div
              key={index}
              ref={(el) => (topicRefs.current[index] = el)}
              className="p-5 md:p-6 lg:p-7 rounded-2xl border border-emerald-200 bg-white shadow-md hover:shadow-lg transition-all duration-200"
            >
              <div
                className="course-content prose prose-emerald max-w-none 
